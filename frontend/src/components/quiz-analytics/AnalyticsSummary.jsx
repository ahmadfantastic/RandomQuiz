...
const AnalyticsSummary = ({ completionStats, timeStats, word_count_stats, cronbach_alpha }) => {
    ...
    return (
        <div className="grid gap-4 md:grid-cols-4">
            ...
            {cronbach_alpha !== undefined && cronbach_alpha !== null && (
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Reliability (Alpha)</CardTitle>
                        <Activity className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{cronbach_alpha.toFixed(2)}</div>
                        <p className="text-xs text-muted-foreground">
